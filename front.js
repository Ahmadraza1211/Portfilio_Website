...
renderTasks() {
        ...
        (task) => `
          <div class="task-item ${this.selectedTasks.some((t) => t.id === task.id) ? "selected" : ""} ${this.completedTasks.has(task.id) ? "completed" : ""}">
            ...
            <div class="task-info-item">
              <div class="task-name-display" data-task-id="${task.id}" onclick="trainer.startRename(${task.id})">${task.name}</div>
              <div class="task-duration">${this.formatTime(task.duration)}</div>
            </div>
            ${this.completedTasks.has(task.id) ? '<span class="checkmark">✓</span>' : ""}
            <div class="task-actions">
              <button class="btn btn-rename btn-small" onclick="trainer.startRename(${task.id})" title="Rename exercise">
                ✎
              </button>
  ...
  startRename(taskId) {
    ...
    const taskItem = document.querySelector(`.task-item .task-name-display[data-task-id="${taskId}"]`);
    const taskActions = taskItem.closest(".task-item").querySelector(".task-actions");
    if (!taskItem || !taskActions) return;

    // Store original name and task ID
    taskItem.dataset.originalName = task.name;
    taskItem.innerHTML = `
      <input type="text" class="task-name-input" value="${task.name}" onblur="trainer.finishRename(${taskId})" onkeypress="if(event.key === 'Enter') trainer.finishRename(${taskId})">
    `;
    taskActions.innerHTML = `
      <button class="btn btn-save btn-small" onclick="trainer.finishRename(${taskId})" title="Save name">
        ✓
      </button>
      <button class="btn btn-stop btn-small" onclick="trainer.cancelRename(${taskId})" title="Cancel rename">
        ×
      </button>
    `;
    taskItem.querySelector(".task-name-input").focus();
  }
  ...
  finishRename(taskId) {
    ...
    const taskItem = document.querySelector(`.task-item .task-name-display[data-task-id="${taskId}"]`);
    const taskActions = taskItem.closest(".task-item").querySelector(".task-actions");
    if (!task || !taskItem || !taskActions) return;

    const input = taskItem.querySelector(".task-name-input");
    const newName = input.value.trim();

    if (newName && newName.length > 0) {
      task.name = newName;
      this.saveTasks();
      this.renderTasks(); // Re-render to revert to default state with new name
      this.showSuccess("Exercise renamed successfully!");
    } else {
      this.showError("Please enter a valid exercise name");
      this.renderTasks(); // Revert to original state on invalid input
    }
  }

  cancelRename(taskId) {
    this.renderTasks(); // Revert to original state by re-rendering
  }
/*
  renderTasks() {
    const taskList = document.getElementById("taskList");
